...
# 7. MEASURE TRANSIT DEPTH
   
 # TRANSIT DEPTH δ
transit_depth = 1.0 - min(binned_lc.flux)
 # TRANSIT DEPTH IN (Parts per Million)
transit_depth_in_ppm = transit_depth * 1000000

# 8. CALCULATE PLANET RADIUS

 # R∗ = 1,065 R☉       Kepler-10 Sun Radius 
 # Rₚ = R∗ · √δ         Kepler-10b Planet Radius
 # 1 R☉ ≈ 109,2 R⊕    Sun Radius to Earth Radii
 # 1 R⊕ = 6 371 Km    Earth Radius in Kilometers <- not so sure

sun_radius = 1.065
planet_radius = sun_radius * math.sqrt(transit_depth)
planet_radius_to_earth_radii = planet_radius * 109.2
planet_radius_kilometers = 6371 * planet_radius_to_earth_radii

print(f"Kepler-10 Sun Radius: {sun_radius} R☉ (Sun radii)")
print(f"Kepler-10b Planet Radius: {planet_radius_to_earth_radii} R⊕ (Earth Radii)")
print(f"Kepler-10b Planet Radius: {planet_radius_kilometers} Km (Kilometers)")


print("\nSuccesfully went trough Pipeline!")
